    ...
    with open(os.path.join(save_path, f'chr{chromosome}_train_{vv}_loss.txt'), 'w') as log:
        ...
        for epoch in range(epochs):
            ...
            for i, (SM0, SM1, SM2) in loop:
                ...
                #v3# non_zero_mask = (_input_SM[:, 2:3, :, :] != 0) & (_input_SA1[:, 2:3, :, :] != 0) & (_input_SA2[:, 2:3, :, :] != 0)
                ...
                #v3# _signal_loss = torch.norm((SMC1A_prediction[:,2:3,:,:] - SA1_prediction[:,2:3,:,:] - SA2_prediction[:,2:3,:,:]).masked_select(non_zero_mask).view(-1), 2)
