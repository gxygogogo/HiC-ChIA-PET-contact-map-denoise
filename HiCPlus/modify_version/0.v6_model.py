...
def validate_model(model, val_loader, va_loss):
    ...
    with torch.no_grad():
        for i, (SM0_val, SM1_val, SM2_val) in val_loader:
            ...
            val_loss = va_loss(SMC1A_prediction_val ,_input_SM) + va_loss(SA1_prediction_val ,_input_SA1) + va_loss(SA2_prediction_val ,_input_SA2)                
...
for i in [21]:
    ...
    # SM_scaler = MinMaxScaler(feature_range=(0., 10.))
    SM_scaler = MinMaxScaler(feature_range=(0., 1.))
    SMC1A_highres = SM_scaler.fit_transform(SMC1A_highres)
    SMC1A_lowres = SM_scaler.fit_transform(SMC1A_lowres)
    SMC1A_signal = SM_scaler.fit_transform(SMC1A_signal)
    ...
    # SM_scaler = MinMaxScaler(feature_range=(0., 10.))
    SA1_highres = SM_scaler.fit_transform(SA1_highres)
    SA1_lowres = SM_scaler.fit_transform(SA1_lowres)
    SA1_signal = SM_scaler.fit_transform(SA1_signal)
    ...
    SA2_highres = SM_scaler.fit_transform(SA2_highres)
    SA2_lowres = SM_scaler.fit_transform(SA2_lowres)
    SA2_signal = SM_scaler.fit_transform(SA2_signal)
    ...
    Y = np.stack((SMC1A_Y[:,2,:,:], SA1_Y[:,2,:,:], SA2_Y[:,2,:,:]), axis=1)

    # 进行训练集和验证集的拆分
    train_highres, val_highres, train_lowres, val_lowres, train_signal, val_signal, train_Y, val_Y = \
        train_test_split(highres_sample, lowres_sample, signal_sample, Y, test_size=0.2, random_state=42)
    ...
    conv_autoencoder = model.conv_autoencoder_pro()
    if use_gpu:
        conv_autoencoder = conv_autoencoder.cuda()

    params_to_optimize = [{'params':conv_autoencoder.parameters()}]
    # optimizer = optim.SGD(params_to_optimize, lr = 0.001)
    # A = torch.nn.Parameter(torch.tensor(2.0), requires_grad=True)
    # params_to_optimize = [{'params':conv_autoencoder.parameters()}, {'params': [A]}]
    optimizer = optim.SGD(params_to_optimize, lr = 0.001, weight_decay= 1e-6)
    ...
    with open(os.path.join(save_path, f'chr{chromosome}_train_{vv}_loss.txt'), 'w') as log:
        ...
        for epoch in range(epochs):
            loop = tqdm(enumerate(zip(train_highres_loader, train_lowres_loader, train_signal_loader, train_Y_loader)), total=len(train_highres_loader))
            for i, (SM0, SM1, SM2, SM3) in loop:
                _highRes = Variable(SM0[0])
                _lowRes = Variable(SM1[0])
                _signal = Variable(SM2[0])
                _re_signal = Variable(SM3[0]).cuda()
                # _raw = Variable(RAW[0])
                ...
                _SM_loss = (1 - F.cosine_similarity(SMC1A_prediction.flatten(), _input_SM.flatten(), dim=0))
                _SA1_loss = (1 - F.cosine_similarity(SA1_prediction.flatten(), _input_SA1.flatten(), dim=0))
                _SA2_loss = (1 - F.cosine_similarity(SA2_prediction.flatten(), _input_SA2.flatten(), dim=0))
                ...
                #v4#
                non_zero_mask = ((_input_SM[:, 0:1, :, :] != 0).to(torch.int) + (_input_SA1[:, 0:1, :, :] != 0).to(torch.int) + (_input_SA2[:, 0:1, :, :] != 0).to(torch.int)) >= 1
                # non_zero_mask = ((_input_SM[:, 2:3, :, :] != 0).to(torch.int) + (_input_SA1[:, 2:3, :, :] != 0).to(torch.int) + (_input_SA2[:, 2:3, :, :] != 0).to(torch.int)) >= 1
                ...
                # A =  (_input_SA1[:, 2:3, :, :].mean() + _input_SA2[:, 2:3, :, :].mean()) / (_input_SM[:, 2:3, :, :].mean())
                # A = torch.nn.Parameter(torch.tensor(2.0))
                B =  (_input_SM[:, 0, :, :].mean()) / (2 * _input_SA1[:, 0, :, :].mean()) 
                C =  (_input_SM[:, 0, :, :].mean()) / (2 * _input_SA2[:, 0, :, :].mean())
                # D = (_input_SM[:, 0, :, :].mean()) / (_input_SM[:, 2, :, :].mean())
                _signal_loss = torch.norm((SMC1A_prediction[:,0:1,:,:] - B * SA1_prediction[:,0:1,:,:] - C * SA2_prediction[:,0:1,:,:]).masked_select(non_zero_mask).view(-1), 2)
                # _signal_loss = torch.norm((SMC1A_prediction[:,2:3,:,:] - B * SA1_prediction[:,2:3,:,:] - C * SA2_prediction[:,2:3,:,:]).masked_select(non_zero_mask).view(-1), 2)
                
                # _signal_loss = torch.norm((SM_encode[:,1:2,:,:] - SA1_encode[:,1:2,:,:] - SA2_encode[:,1:2,:,:]).masked_select(non_zero_mask).view(-1), 2)
                # loss = _SM_loss + _SA1_loss + _SA2_loss
                loss = _SM_loss + B *_SA1_loss + C * _SA2_loss + _signal_loss
                # _signal_loss = torch.norm((_input_SM[:,0:1,:,:] - B * _input_SA1[:,0:1,:,:] - C * _input_SA2[:,0:1,:,:]).masked_select(non_zero_mask).view(-1), 2)
            ...
            print(f'Epoch [{epoch + 1}/{epochs}], Training Loss: {running_loss / (len(train_highres_loader) - 1)}, SMC1A Loss: {SM_loss}, SA1 Loss: {SA1_loss}, SA2 Loss: {SA2_loss}, Signal Loss: {signal_loss}')
            ...
            # log.write(str(epoch) + '\t' + str(running_loss/(len(train_highres_loader)-1)) + '\t' + f'{val_loss}' + '\t' + f'{SM_loss}' + '\t' + f'{SA1_loss}' + '\t' + f'{SA2_loss}' + '\n')
